...
export function createD3Lightbox(options: LightboxOptions = {}): LightboxInterface {
  ...
  const container = d3.select<HTMLElement, unknown>(config.containerSelector)
  let box = container.select<HTMLDivElement>('#lightbox')
  if (box.empty()) {
  ...
  let imageContainer = box.select<HTMLDivElement>('.lightbox-image-container')
  ...
  let controls = box.select<HTMLDivElement>('.lightbox-controls')
  if (controls.empty()) {
  ...
  if (controls.select('.lightbox-close').empty()) {
    controls.append('button').attr('class', 'lightbox-close').text('×').on('click', closeLightbox)
  }
  ...
  const previousWillChange = document.body.style.willChange
  ...
  imageContainer.on('touchstart', function (event) {
    ...
    if (event.touches.length === 2) {
      ...
      const img = imageContainer.select<HTMLImageElement>('img')
      ...
      const touchMoveHandler = function (moveEvent: TouchEvent) {
        ...
        img
          ...
          .style(
            'transform',
            `translate(${currentTranslateX}px, ${currentTranslateY}px) scale(${scaleFactor})`,
          )
      }
      ...
      const touchEndHandler = function () {
        ...
        const transform = img.style('transform') || ''
        const scaleMatch = transform.match(/scale\(([0-9.]+)\)/)
        const finalScale = scaleMatch && scaleMatch[1] ? parseFloat(scaleMatch[1]) : 1
        ...
        if (finalScale < config.minZoom * 1.1) {
          ...
          img
            .style(
              'transition',
              `transform ${config.transitionDuration / 2}ms cubic-bezier(0.2, 0, 0.1, 1)`,
            )
          ...
          img
            ...
            .style(
              'transform',
              `translate(${currentTranslateX}px, ${currentTranslateY}px) scale(${config.maxZoom})`,
            )
          ...
          later(constrainPanning, config.transitionDuration / 2)
        } else {
      ...
      // Set up handlers
      d3.select(window).on('touchmove.pinchzoom', touchMoveHandler)
      d3.select(window).on('touchend.pinchzoom', touchEndHandler)
      return
    }
  ...
  imageContainer.on('touchmove', function (event) {
    ...
    if (img.classed('zoomed')) {
      ...
      const newX = currentTranslateX + transformX
      const newY = currentTranslateY + transformY
    ...
    if (Math.abs(transformX) > Math.abs(transformY)) {
      ...
      if (
        (currentIndex === 0 && transformX > 0) ||
        (currentIndex === images.length - 1 && transformX < 0)
      ) {
  ...
  imageContainer.on('touchend', function () {
    ...
    img.style(
      'transition',
      `transform ${config.transitionDuration}ms cubic-bezier(0.2, 0, 0.1, 1), opacity ${config.transitionDuration}ms ease`,
    )
    ...
    if (Math.abs(deltaX) > Math.abs(deltaY)) {
      ...
      if (Math.abs(deltaX) >= config.dragThreshold) {
        if (deltaX > 0) {
          ...
          img.style('transform', `translateX(100%)`).style('opacity', '0')
          transitionToImage(currentIndex > 0 ? currentIndex - 1 : images.length - 1, 'prev')
        } else {
        ...
        img.style('transform', 'translateX(0)').style('opacity', '1')
      }
    } else if (Math.abs(deltaY) > config.dragThreshold) {
      // Vertical swipe - close lightbox
      img.style('transform', `translateY(${deltaY > 0 ? '100%' : '-100%'})`).style('opacity', '0')
      later(closeLightbox, config.transitionDuration / 2)
    } else {
      // Not enough swipe distance - reset position
      img.style('transform', 'translateX(0)').style('opacity', '1')
    }
  ...
  function constrainPanning() {
    ...
    if (adjustedX !== currentTranslateX || adjustedY !== currentTranslateY) {
      img
        .style(
          'transition',
          `transform ${config.transitionDuration / 3}ms cubic-bezier(0.2, 0, 0.1, 1)`,
        )
  ...
  function transitionToImage(newIndex: number, direction: 'prev' | 'next') {
    if (!box.classed('active') || !images[newIndex] || transitioning) return
    transitioning = true
    const currentImg = imageContainer.select<HTMLImageElement>('img')
    const newImgData = images[newIndex]
    const offset = direction === 'prev' ? -100 : 100
    const newImg = imageContainer
      .append('img')
      .attr('src', config.imagePathPrefix + newImgData.data.image)
      .attr('alt', newImgData.data.alt || newImgData.data.title || 'Image')
    ...
    newImg.style('transform', 'translateX(0)').style('opacity', '1')
    currentImg.style('transform', `translateX(${-offset}%)`).style('opacity', '0')
    later(() => {
      currentImg.remove()
  ...
  function updateImage() {
    ...
    const img = imageContainer
      ...
      .attr('alt', imageData.data.alt || imageData.data.title || 'Image')
      .style('max-width', isMobile ? '100%' : '90%')
      .style('max-height', isMobile ? '100%' : '90%')
      .style('object-fit', 'contain')
      ...
      .style(
        'transition',
        `opacity ${config.transitionDuration}ms ease, transform ${config.transitionDuration}ms cubic-bezier(0.2, 0, 0.1, 1)`,
      )
      .on('click', function (event) {
        event.stopPropagation()
      })
  ...
  function showCaption(imageData: LightboxImage) {
    ...
    if (captionText) {
      const caption = imageContainer.append('div').attr('class', 'image-caption').text(captionText)
      // Animate caption entry after a short delay
      later(() => {
        caption.style('opacity', '1').style('transform', 'translateY(0)')
      }, 100)
    }
  }

  function preloadAdjacentImages() {
    let preloadContainer = box.select<HTMLDivElement>('.preload-container')
    if (preloadContainer.empty()) {
      preloadContainer = box
        .append('div')
  ...
  function closeLightbox() {
    ...
    transitioning = false
    previousFocus?.focus()
    box.classed('active', false).attr('aria-hidden', 'true')
    d3.select('body').style('overflow', previousOverflow).style('will-change', previousWillChange)
    later(() => {
      imageContainer.selectAll('img').remove()
      imageContainer.select('.image-caption').remove()
    }, config.transitionDuration)
  ...
  return {
    ...
    setImages: function (imgArray: LightboxImage[]) {
      images = imgArray
    },
    destroy: () => {
      clearPending()
      clearTimeout(box.property('hideTimeout'))
      d3.select(window).on('.pinchzoom', null)
      d3.select('body')
        .on('.lightbox', null)
        .style('overflow', previousOverflow)
        .style('will-change', previousWillChange)
      box.selectAll('*').interrupt()
      box.remove()
    },
  }
  ...
  function openLightbox(clickedImage: LightboxImage, allImages: LightboxImage[]) {
    ...
    box
      ...
      .attr('aria-label', 'Project images')
      .attr('aria-hidden', 'false')
      .attr('tabindex', -1)
    box.node()?.focus()
    d3.select('body').style('will-change', 'transform').style('overflow', 'hidden')
    updateImage()
    if (config.preloadImages) preloadAdjacentImages()
  }
}

/**
 * D3 Lightbox - Improved version (lightbox3b.js)
 *
 * Features:
...
function createD3Lightbox(options = {}) {
  ...
  // Touch event handling - completely rewritten for reliable detection
  ...
  function showCaption(imageData) {
  ...
  function getContainedSize(imgElement) {
	const naturalWidth = imgElement.naturalWidth;
	const naturalHeight = imgElement.naturalHeight;
	const containerRect = imageContainer.node().getBoundingClientRect();
	const maxContainerWidth = isMobile ? containerRect.width : containerRect.width * 0.9;
	const maxContainerHeight = isMobile ? containerRect.height : containerRect.height * 0.9;
	const aspectRatio = naturalWidth / naturalHeight;
	let width = maxContainerHeight * aspectRatio;
	let height = maxContainerHeight;
	if (width > maxContainerWidth) {
	  width = maxContainerWidth;
	  height = maxContainerWidth / aspectRatio;
	}
	return [width, height];
  }
  ...
  return {
	...
	setImages: function(imgArray) { images = imgArray; }
  };

  function openLightbox(clickedImage, allImages) {
	images = allImages;
...
// Export as module if applicable
if (typeof module !== 'undefined' && module.exports) {
  module.exports = createD3Lightbox;
}